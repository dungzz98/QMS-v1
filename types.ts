...
export interface DecontaminationLog {
  id: string;
  date: string; // YYYY-MM-DD
  time: string; // HH:MM
...
export interface NonConformity {
  ...
  category: 'pre-analytical' | 'analytical' | 'post-analytical' | 'system' | 'safety' | 'supplier' | 'equipment' | 'other';
...
export interface EQAResult {
  id: string;
  eqaMaterialId: string;
  testParameterId: string;
  date: string; // ISO string
  labValue: number;
  peerGroupMean: number;
  peerGroupSd: number;
  evaluation?: 'acceptable' | 'warning' | 'unacceptable';
  reportFileData?: string; // base64
  reportFileName?: string;
  reportFileType?: string; // MIME type
  notes?: string;
  recordedBy: string;
}

export interface EQASchedule {
  id: string;
  eqaMaterialId: string;
  sampleReceptionDate: string; // YYYY-MM-DD
  resultDueDate: string; // YYYY-MM-DD
...
// New types for Assessment Module
...
export type AssessmentStatus = 'Đạt' | 'Không đạt' | 'Không áp dụng';
...
export interface Assessment {
    ...
    findings: { [criterionId: string]: AssessmentFinding }; // Key is criterionId
}

// New type for Information Management
export interface SystemEvent {
  id: string;
  timestamp: string; // ISO
  type: 'error' | 'maintenance' | 'update' | 'backup';
  description: string;
  user: string;
...
export type WarehouseSubPage = 'dashboard' | 'inventory' | 'stockIn' | 'stockOut' | 'onInstrument' | 'preparationLog' | 'reports' | 'planning' | 'physicalCount' | 'disposal';

export type SafetySubPage = 'dashboard' | 'access' | 'areaEnv' | 'equipEnv' | 'waterConductivity' | 'incidents' | 'manageItems' | 'decontamination';