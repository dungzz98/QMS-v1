...
import { TestParameter, ChemicalMaster, Instrument, RoomLocation, StorageLocation, ControlMaterial, EQAMaterial, DocumentCategory, User, WorkSchedule, Holiday, ControlLotTarget } from '../types';
import { PlusIcon } from './icons/PlusIcon';
import { EditIcon } from './icons/EditIcon';
import { TrashIcon } from './icons/TrashIcon';
import { ImportIcon } from './icons/ImportIcon';
import ChemicalMasterImportModal from './ChemicalMasterImportModal';
import TestParameterImportModal from './TestParameterImportModal';


type ActiveTab = 'tests' | 'chemicals' | 'equipment' | 'locations' | 'storage' | 'iqc' | 'targets' | 'eqa' | 'documents' | 'users' | 'schedule';

type ModalType = 'testParameter' | 'chemicalMaster' | 'instrument' | 'roomLocation' | 'storage' | 'controlMaterial' | 'controlLotTarget' | 'eqaMaterial' | 'documentCategory';
...
interface SettingsPageProps {
    ...
    onOpenChemicalMasterImport: () => void;
    onImportChemicalMasters: (file: File, mode: 'replace' | 'append') => void;
    onDownloadChemicalTemplate: () => void;
    // Props for test parameter import
    onOpenTestParameterImport: () => void;
    onImportTestParameters: (file: File, mode: 'replace' | 'append') => void;
    onDownloadTestTemplate: () => void;
...
const SettingsPage: React.FC<SettingsPageProps> = (props) => {
    const { 
        testParameters, chemicalMasters, instruments, roomLocations, storageLocations, controlMaterials, controlLotTargets, eqaMaterials, documentCategories,
        onOpenModal, onDeleteItem, onOpenChemicalMasterImport, onImportChemicalMasters, onDownloadChemicalTemplate, onOpenTestParameterImport, onImportTestParameters, onDownloadTestTemplate,
    ...
    const filteredData = useMemo(() => {
        ...
        return {
            ...
            targets: controlLotTargets.filter(t => {
                const testName = testParameters.find(tp => tp.id === t.testParameterId)?.name || '';
                const controlName = controlMaterials.find(cm => cm.id === t.controlMaterialId)?.name || '';
                const controlLot = controlMaterials.find(cm => cm.id === t.controlMaterialId)?.lotNumber || '';
                return testName.toLowerCase().includes(lower) || controlName.toLowerCase().includes(lower) || controlLot.toLowerCase().includes(lower);
            }),
    ...
    }, [filterText, testParameters, chemicalMasters, instruments, roomLocations, storageLocations, controlMaterials, controlLotTargets, eqaMaterials, documentCategories, users]);
    ...
    const getAddButtonText = () => {
        switch (activeTab) {
            ...
            case 'targets': return 'Thêm Target';
    ...
    const getModalType = (): ModalType | 'user' => {
         switch (activeTab) {
            ...
            case 'targets': return 'controlLotTarget';
    ...
    const renderTable = () => {
        switch(activeTab) {
            ...
            case 'targets': {
                const testMap = new Map(testParameters.map(t => [t.id, t.name]));
                const controlMap = new Map(controlMaterials.map(c => [c.id, `${c.name} ${c.level} (Lô: ${c.lotNumber})`]));
                return (
                    <table className="min-w-full"><thead className="bg-white"><tr className="border-b-2 border-black"><th className="px-4 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Xét nghiệm</th><th className="px-4 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Vật liệu Control</th><th className="px-4 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Mean</th><th className="px-4 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">SD</th><th className="px-4 py-3 text-left text-xs font-bold text-black uppercase tracking-wider">Hành động</th></tr></thead>
                        <tbody className="bg-white divide-y divide-gray-200">{filteredData.targets.map(i => <tr key={i.id} className="hover:bg-gray-50">
                            <td className="px-4 py-3 font-medium text-black">{testMap.get(i.testParameterId) || 'N/A'}</td>
                            <td className="px-4 py-3 text-gray-700">{controlMap.get(i.controlMaterialId) || 'N/A'}</td>
                            <td className="px-4 py-3 text-gray-700">{i.mean}</td>
                            <td className="px-4 py-3 text-gray-700">{i.sd}</td>
                            <td className="px-4 py-3"><div className="flex items-center gap-3"><button onClick={() => onOpenModal('controlLotTarget', i)} className="text-gray-700 hover:text-black"><EditIcon /></button><button onClick={() => onDeleteItem('controlLotTarget', i.id)} className="text-gray-700 hover:text-black"><TrashIcon /></button></div></td>
                        </tr>)}
                        {filteredData.targets.length === 0 && (
                            <tr><td colSpan={5} className="text-center p-8 text-gray-500">Chưa có target nào.</td></tr>
                        )}
                        </tbody></table>
                );
            }
            ...
            <div className="bg-white p-6 rounded-lg border border-gray-200 text-black">
                <div className="flex flex-wrap justify-between items-center mb-4 gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        {activeTab === 'tests' && <button onClick={onOpenTestParameterImport} className="inline-flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-md text-sm hover:bg-gray-100"><ImportIcon/>Nhập Excel</button>}
                        {activeTab === 'chemicals' && <button onClick={onOpenChemicalMasterImport} className="inline-flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-md text-sm hover:bg-gray-100"><ImportIcon/>Nhập Excel</button>}
                ...
                <div className="flex flex-wrap gap-2 mb-4 border-b border-gray-200 pb-4">
                    ...
                    <TabButton tab="targets" label="Target Nội kiểm" />
