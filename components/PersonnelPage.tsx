...
import { PersonnelProfile, JobRole, TraineeProfile, TrainingCourse, TrainingRecord, Competency, CompetencyAssessment, LeaveRecord, AttendanceRecord, DutyAssignment, TaskAssignment, PersonnelAssignmentHistory, KpiScore, WorkSchedule, Holiday, OrganizationUnit } from '../types';
...
const StaffList: React.FC<{ personnel: PersonnelProfile[], jobRoles: JobRole[], filterText: string, onViewDetails: (p: PersonnelProfile) => void, onEdit: (p: PersonnelProfile) => void, onDelete: (id: string) => void, focusedItemId: string | null }> = ({ personnel, jobRoles, filterText, onViewDetails, onEdit, onDelete, focusedItemId }) => {
    ...
    const filteredPersonnel = useMemo(() => {
        const sorted = [...personnel].sort((a,b) => a.fullName.localeCompare(b.fullName, 'vi'));
...
interface PersonnelPageProps {
  ...
  // New props from OrganizationPage
...
type ActiveTab = 'staff' | 'trainees' | 'training' | 'competency' | 'attendance' | 'assignment' | 'kpi' | 'history';
...
const PersonnelPage: React.FC<PersonnelPageProps> = (props) => {
  ...
  const addButtonText: Record<ActiveTab, string | null> = {
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg text-black">
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-1">Quản lý hồ sơ, đào tạo, chấm công và đánh giá năng lực nhân sự.</p>
        </div>
       ...
       <div className="flex flex-wrap justify-between items-center mb-4 gap-4 border-b border-gray-200 pb-4">
            <div className="flex gap-2 flex-wrap">
                ...
                <TabButton tabId="training" label="Đào tạo" icon={<CertificateIcon />} />
                <TabButton tabId="competency" label="Năng lực" icon={<CheckBadgeIcon />} />
      ...
      <div className="overflow-x-auto">
        ...
        {activeTab === 'attendance' && <AttendanceCalendar 
            ...
            dutyAssignments={props.dutyAssignments}
            workSchedule={props.workSchedule} 
            holidays={props.holidays} 
            onAddLeave={props.onAddLeave}
            onAddOrUpdateAttendance={props.onAddOrUpdateAttendance}
        />}
        {activeTab === 'assignment' && <AssignmentPage 
            ...
            onAddDutyAssignment={props.onAddDutyAssignment}
            onDeleteDutyAssignment={props.onDeleteDutyAssignment}
            onAddTaskAssignment={props.onAddTaskAssignment}
        />}
        {activeTab === 'kpi' && <KpiPage 
             ...
             holidays={props.holidays}
             workSchedule={props.workSchedule}
             kpiScores={props.kpiScores}
             onAddOrUpdateKpiScore={props.onAddOrUpdateKpiScore}
        />}
        {activeTab === 'history' && <AssignmentHistoryPage 
            history={props.personnelAssignmentHistory}
            personnel={props.personnel}
            organizationUnits={props.organizationUnits}
            onOpenMovePersonnel={props.onOpenMovePersonnel}
        />}
