...
const EquipmentOperationPage: React.FC<EquipmentOperationPageProps> = ({ equipment, onUpdate }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 h-[70vh]">
      <div className="md:col-span-1 border rounded-lg p-2 flex flex-col bg-white">
        <h3 className="text-md font-semibold p-2 text-slate-700">1. Chọn thiết bị</h3>
      ...
      <div className="md:col-span-2 flex flex-col bg-white p-6 rounded-lg shadow-inner border">
          {selectedEquipment ? (
            <>
                <h3 className="text-lg font-semibold text-slate-800 mb-2">Quy trình Vận hành cho: <span className="text-blue-700">{selectedEquipment.name}</span></h3>
                <p className="text-sm text-slate-500 mb-4">Soạn thảo các bước vận hành, hướng dẫn xử lý sự cố, hoặc quy trình thao tác chuẩn (SOP) tại đây.</p>
                <textarea
                    value={procedureText}
                    onChange={(e) => setProcedureText(e.target.value)}
                    placeholder="Nhập nội dung quy trình vận hành tại đây..."
                    className="w-full flex-grow p-3 border rounded-md resize-none focus:ring-2 focus:ring-blue-500 text-sm leading-relaxed"
                />
