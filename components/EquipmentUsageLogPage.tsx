...
const EquipmentUsageLogPage: React.FC<EquipmentUsageLogPageProps> = ({ 
    logs, equipment, personnel, onAdd, onEdit, currentUser, onExportUsageLogToDoc, accessLogs, equipmentFilter, onEquipmentFilterChange,
              ...
              <tr key={log.id}>
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            onClick={() => onCreateNonConformity(log)}
                            disabled={!log.incidents || !!log.nonConformityId}
                            className="p-1 rounded-full text-orange-600 hover:text-orange-800 disabled:text-gray-300 disabled:cursor-not-allowed"
                            title={
                                log.nonConformityId 
                                    ? `Sự cố này đã được chuyển thành phiếu SKPH.`
                                    : (!log.incidents ? 'Không có sự cố để chuyển' : 'Chuyển sự cố này thành phiếu Sự không phù hợp')
                            }
                        >
                            <DocumentAlertIcon className="w-5 h-5" />
                        </button>
                    </div>
                </td>
              </tr>
