// FIX: Add useEffect and useRef for scrolling to focused item
...
interface InventoryTableProps {
  ...
  // FIX: Add focusedItemId to props to allow highlighting search results
...
const InventoryTable: React.FC<InventoryTableProps> = ({ chemicals, onEdit, onDelete, onGetSafetyInfo, onOpenAdjustModal, onOpenUsageLog, onOpenBarcodeModal, onOpenSafetyDoc, focusedItemId }) => {
  ...
  // FIX: Add ref for scrolling to focused item
        ...
        <table className="min-w-full divide-y divide-slate-200">
          ...
          <tbody className="bg-white divide-y divide-slate-200">
            {sortedChemicals.length > 0 ? sortedChemicals.map(chemical => {
              ...
              return (
                <tr key={chemical.id} 
                    // FIX: The ref callback must not return a value. Wrapped the assignment in curly braces to create a block statement.
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                    <div className="flex items-center gap-1">
                        {status.icon && (
                            // FIX: Wrap WarningIcon in a span with a title attribute to show tooltip, as the component does not accept a 'title' prop.
