...
import { Chemical, ChemicalMaster, DisposalRecord, Instrument, ManualPreparationLog, OnInstrumentStock, PlanningSlip, StorageLocation, User, MonitoredArea, MonitoredEquipment, AreaEnvironmentLog, EquipmentTemperatureLog, WaterSource, WaterConductivityLog, WarehouseSubPage } from '../types';
...
interface WarehouseManagementPageProps {
    ...
    // FIX: Add missing prop from App.tsx
...
const WarehouseManagementPage: React.FC<WarehouseManagementPageProps> = (props) => {
    const [activeSubPage, setActiveSubPage] = useState<WarehouseSubPage>('dashboard');
    const [isMenuOpen, setIsMenuOpen] = useState(false);
    const menuRef = useRef<HTMLDivElement>(null);

    const subPages: { id: WarehouseSubPage; label: string }[] = [
    ...
    const handleSelectPage = (page: WarehouseSubPage) => {
    ...
    const renderContent = () => {
        switch (activeSubPage) {
            case 'dashboard':
                ...
                />;
            case 'inventory':
                // FIX: Pass focusedItemId prop to InventoryTable
