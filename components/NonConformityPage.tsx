      ...
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
       ...
       <div className="flex flex-wrap justify-between items-center mb-4 gap-4 border-b border-gray-200 pb-4">
            <div className="flex gap-2">
                <TabButton tabId="list" label="Sổ Sự không phù hợp" icon={<ListBulletIcon />} />
                <TabButton tabId="corrective_actions" label="Theo dõi HĐ Khắc phục" icon={<ClipboardDocumentCheckIcon />} />
                <TabButton tabId="preventive_actions" label="Theo dõi HĐ Phòng ngừa" icon={<ShieldCheckIcon />} />
            </div>
      ...
      {activeTab === 'list' && (
        <div className="overflow-x-auto">
            ...
            <thead className="bg-slate-50">
                <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">Mã SKPH</th>
            ...
            <tbody className="bg-white divide-y divide-slate-200">
                ...
                const isFocused = item.id === focusedNcId;
                return (
                <tr key={item.id} className={`transition-colors duration-300 ${isFocused ? 'animate-pulse-yellow' : 'hover:bg-slate-50'}`}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-800">
                        <button onClick={() => handleNavigate('list', item.id)} className="hover:underline" title="Nhấn để highlight dòng này">
                            {item.ncId}
                        </button>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">
                      ...
                      {paReport ? (
                        <button onClick={() => handleNavigate('preventive_actions', paReport.id)} className="text-green-600 hover:underline block">
                         ...
                         <button
                            onClick={() => onAddOrUpdatePreventiveAction({ 
                                nonConformityId: item.id,
                                problemDescription: `Từ SKPH ${item.ncId}: ${item.description}`,
                                riskFactors: item.rootCauseAnalysis || '',
                                preventiveMeasures: item.preventiveAction || '',
                            })}
                            className="text-xs text-green-700 bg-green-100 hover:bg-green-200 rounded px-2 py-1 mt-1"
                            title="Tạo phiếu HĐ Phòng ngừa từ SKPH này"
                        >
                            Tạo HĐPN
                        </button>
                      ) : null}
